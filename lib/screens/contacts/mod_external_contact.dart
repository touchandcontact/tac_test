...
class ModExternalContactModal extends StatefulWidget {
  const ModExternalContactModal({
    super.key,
    required this.contactMod,
    required this.idContattoDaLista,
  });

  final UserContactInfoModel contactMod;
  final int idContattoDaLista;

  @override
  ModExternalContactModalState createState() => ModExternalContactModalState();
}

class ModExternalContactModalState extends State<ModExternalContactModal> {
  ...
  String headerText = "";
  File? pickedImage;
  late UserContactInfoModel contact;
  ValueNotifier<List<ExternalContactAddressCustom>> addressesNotifier =
      ValueNotifier<List<ExternalContactAddressCustom>>([]);
  ...
  void initState() {
    ...
    if (_addressController.text.isNotEmpty &&
        _addressController.text.length > 32) {
      _addressController.text =
          "${_addressController.text.substring(0, 32)}...";
    }
    _addressController.text = _addressController.text;
    addressesNotifier.value = contact.addresses != null &&
            contact.addresses!.isNotEmpty
        ? contact.addresses!
            .map<ExternalContactAddressCustom>((e) =>
                ExternalContactAddressCustom(
                    e,
                    TextEditingController(
                        text: e.address != null && e.address!.length > 32
                            ? "${e.address?.substring(0, 32)}..."
                            : e.address! ?? "")))
            .toList()
        : [];
  ...
  Future save(BuildContext context) async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      try {
        await editExternalContact(
            user.tacUserId,
            contact.notes ?? "",
            externalContact,
            pickedImage,
            addressesNotifier.value.isNotEmpty
                ? addressesNotifier.value.map((e) => e.obj).toList()
                : [],
            telephonesNotifier.value,
            widget.idContattoDaLista);
  ...
  updateTextEditingController(){
    if (addressesNotifier.value.isNotEmpty) {
      addressesNotifier.value.forEach((element) {
        if (element.textEditingController.text.isNotEmpty) {
          if (element.textEditingController.text.length > 28) {
            element.textEditingController.text =
                "${element.textEditingController.text.substring(0, 28)}..."
                    ;
          }
        }
      });
  ...
  void addAddress() {
    ...
    addressesNotifier.value = [
      ...addressesNotifier.value,
      ExternalContactAddressCustom(
          ExternalContactAddress(), TextEditingController())
    ];
  }

  void removeAddress(ExternalContactAddress item) {
    updateTextEditingController();
    addressesNotifier.value.removeWhere((element) => element.obj == item);
                  ...
                  child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          Expanded(
                            child: IconButton(
                                onPressed: () => Navigator.pop(context, isMod),
                      ...
                      children: [
                        ...
                        Center(
                            child: ProfileImagePicker(
                                initialImage: widget.contactMod.profileImage,
                          ...
                          child: Form(
                              ...
                              child: Column(children: [
                                ...
                                ValueListenableBuilder(
                                    valueListenable: telephonesNotifier,
                                    builder: (context, valueNotifier, child) =>
                                        getTelephones(valueNotifier)),
                                const SizedBox(
                                  height: 10,
                                ),
                                Focus(
  ...
  Widget getAddresses(List<ExternalContactAddressCustom> addressList) {
    ...
    return Column(
      ...
      return Padding(
          ...
          child: Focus(
            onFocusChange: (hasFocus) {
              if (hasFocus) {
                entry.value.textEditingController.text =
                    entry.value.obj.address ?? "";
              } else {
                if (entry.value.textEditingController.text.length > 28) {
                  entry.value.textEditingController.text =
                      "${entry.value.textEditingController.text.substring(0, 28)}..."
                          ;
                }
              }
            },
            child: InputText(
                suffixIcon: Padding(
                  ...
                  child: IconButton(
                      onPressed: () => removeAddress(entry.value.obj),
                      icon: Icon(
                        Icons.delete,
                ...
                controller: entry.value.textEditingController,
                label: "${AppLocalizations.of(context)!.address} ${entry.key + 2}",
                onChange: (e) => entry.value.obj.address = e),
          ));
    }).toList());
  }
...
class ExternalContactAddressCustom {
  ExternalContactAddress obj;
  TextEditingController textEditingController;

  ExternalContactAddressCustom(this.obj, this.textEditingController);
}
