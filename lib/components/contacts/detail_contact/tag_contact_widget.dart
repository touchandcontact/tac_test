...
class TagContactWidget extends StatefulWidget {
  List<Tag> listaTagContact;
  int idContact;
  bool isAggiungiContatto;

  TagContactWidget({Key? key,required this.listaTagContact,required this.idContact, this.isAggiungiContatto = false}) : super(key: key);

  @override
  State<TagContactWidget> createState() => _TagContactWidgetState();
}

class _TagContactWidgetState extends State<TagContactWidget> {
  ...
  _saveTag() async {
    ...
    try{
      final response = await editContactTags(widget.idContact,_temporaneyListTag);
      if(response){
        // widget.listaTagContact = await _refreshTagList();
                            ...
                            return Padding(
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                  children: [
                                    _generateTextWidget(e.tag, index == 0 ? Theme.of(context).textTheme.headline2!.color! : Theme.of(context).textTheme.headline1!.color!,fontWeight: FontWeight.w600),
                                    Checkbox(
                                        fillColor: MaterialStateColor.resolveWith((states) {
                                          if(!states.contains(MaterialState.selected)){
                                            return Theme.of(context)
                                        ...
                                        shape: const CircleBorder(),
                                        value: _temporaneyListTag.any((element) => element.tag == e.tag), onChanged: (bool? val){
                                      if(val!){
                                  ...
                                  .map<Widget>((e) => Padding(
                                        ...
                                        child: FilterChip(
                                          ...
                                          onSelected: (bool value) {
                                            if(!_temporaneyListTag.contains(e)){
                                              setState(() {
                                                _temporaneyListTag.add(e);
                                              });
                                            }
                ...
                children: [
                  Expanded(
                    child: OutlinedButton(
                      ...
                      onPressed: () => widget.isAggiungiContatto ? _saveTagPerAggiungiContatto() : _saveTag(),
                      child: Padding(
